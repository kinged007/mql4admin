...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($user_id) ) 
{
    ...
    if( time() > strtotime( 'monday this week' ) && $row['updated_at'] < strtotime( 'monday this week' ) ){
        $data_to_update['start_balance_week'] = $data_to_update['balance'];
    }
    if( time() > strtotime( 'today' ) && $row['updated_at'] < strtotime( 'today' ) ){
        $data_to_update['start_balance_day'] = $data_to_update['balance'];
    }
    if( time() > strtotime(date('Y-m-1')) && $row['updated_at'] < strtotime(date('Y-m-1')) ){
